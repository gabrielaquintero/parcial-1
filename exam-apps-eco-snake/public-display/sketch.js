...
socket.on('next-move',direction =>{
    let {moves}= direction

    switch(moves){
        case 1:
            let{controllerX}=direction
            positionX= (controllerX*windowWidth)+1
            break;
            case 2:
                let{controllerY}=direction
            positionX= (controllerY*windowHeight)-1
            break
    }
})
